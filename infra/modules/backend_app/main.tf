# ECR、ECS Fargate、ALB。TLS終端はCloudFrontで行い(06 §4)、ALBは
# CloudFrontのorigin-facingプレフィックスリストからのHTTPのみ受け付ける。
...
# ---- 機密値の入れ物(値はTerraform外で設定し、stateへ残さない) ----
...
# CloudFrontとALBの間は初期構成ではHTTP(:80)とする。独自ドメインとACM証明書の
# 確定後にHTTPSリスナーへ切り替える(infra/README.md、06 Phase 0)。
