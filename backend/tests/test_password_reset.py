...
def test_confirm_reset_with_valid_token_updates_password(client, auth_headers, monkeypatch):
    ...
    res = client.post("/auth/password-reset/confirm", json={"token": token, "new_password": "new-password-456"})
    assert res.status_code == 200
...
def test_confirm_reset_token_is_single_use(client, auth_headers, monkeypatch):
    ...
    res = client.post("/auth/password-reset/confirm", json={"token": token, "new_password": "another-password"})
    assert res.status_code == 400


def test_confirm_reset_with_invalid_token_returns_400(client):
    res = client.post("/auth/password-reset/confirm", json={"token": "not-a-real-token", "new_password": "whatever123"})
    assert res.status_code == 400
