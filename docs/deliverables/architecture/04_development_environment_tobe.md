...
DBスキーマは今後Alembic等でversion管理し、`create_all()`を本番更新手段にしない。本番DBはRDS PostgreSQLに決定したため([ADR-003](adr/ADR-003-aws-container-deployment.md)、[AWSデプロイToBe設計](06_aws_deployment_tobe.md))、ローカル・CIはコンテナのPostgreSQLを用いた統合テストへ移行する。
...
- [ ] Alembic等のversioned migrationを導入する(PostgreSQL移行の前提)
