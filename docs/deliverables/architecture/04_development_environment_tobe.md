...
clone後のセットアップ、実装、検証をLinux/WSL上で再現可能にする。[ADR-003](adr/ADR-003-aws-container-deployment.md)により、開発・検証経路もコンテナ(Docker Compose)を採用する方針へ転換した(従来のコンテナ不採用方針は撤回)。現行実装では、backendとPostgreSQLはDocker Compose(root `compose.yaml`)で起動し、frontendおよびlint・test・E2E等はOSへ言語ランタイムを個別導入せず、リポジトリ内の`.tools/`の固定版toolchainで実行する。
...
- [ ] CI・E2Eをコンテナ構成で実行し、`.tools/`方式の併用範囲(lint等)と廃止時期を決める
...
- [ ] 開発・本番が同一のbackend Dockerfileを使う
