...
`main.py`は全APIエンドポイントを`routers/`配下のドメイン別モジュールへ委譲する。FastAPIアプリ生成、lifespan、CORS、ルーター登録に加え、共通401/403/429を補うOpenAPI生成を担うコンポジションルートである。初期データ投入は`seed.py`へ分離している。
...
classDiagram
    class main {
        <<module>>
        コンポジションルート(app生成・CORS・OpenAPI・local/test schema準備・ルーター登録)
    }
    class config {
        <<module>>
        環境変数の集約・型変換・環境別起動検査
    ...
    class services_checkout {
        <<module>>
        カート・在庫・クーポン検証、見積もり、注文確定トランザクション
    }
    class services_order_status {
        <<module>>
        注文ステータスと許可遷移
    }
    class services_order_actions {
        <<module>>
        注文取消/返品承認時の巻き戻し(reverse_order)
    }
    class models {
        <<module>>
        SQLAlchemyモデル定義
    ...
    routers ..> services_checkout : 依存(orders, payment, coupons)
    routers ..> services_order_status : 依存(admin_orders)
    routers ..> services_order_actions : 依存(orders, admin_orders)
    routers ..> rate_limit : 依存(users、2026-07-13追加)
    routers ..> logging_config : 依存(payment)
    services_checkout ..> services_order_calc : 依存
    services_checkout ..> models : 依存
    services_order_actions ..> stripe_client : 依存
    services_order_actions ..> models : 依存
    services_order_actions ..> logging_config : 依存
    email_utils ..> config : 依存(配送方式・SMTP)
    email_utils ..> logging_config : 依存
    rate_limit ..> logging_config : 依存
    stripe_client ..> config : 依存
    auth ..> models : 依存
    auth ..> database : 依存
    auth ..> config : 依存(SECRET_KEY)
    database ..> config : 依存(DATABASE_URL)
    logging_config ..> config : 依存(LOG_LEVEL)
    models ..> database : 依存
    seed ..> auth : 依存
    seed ..> models : 依存
    seed ..> database : Sessionを呼び出し元から受領
```

- `schemas`・`config`・`services/order_calc.py`は他の自作モジュールに依存していない(外部ライブラリ・標準ライブラリのみに依存する)。`config`を環境設定の下位境界とし、`main`・`auth`・`database`・`email_utils`・`logging_config`・`stripe_client`が参照する。
...
- **`main.py`**: FastAPIアプリのコンポジションルート。アプリ生成、lifespan、CORS、ルーター登録、共通認証/認可/レート制限レスポンスをOpenAPIへ追加する`custom_openapi`を担う。業務エンドポイントやシードデータの定義は持たない。
- **`seed.py`**: 明示的な`make seed`、test fixture、E2E entry pointから受け取ったDBセッションへ、空の場合に限り商品・クーポン・開発用管理者を投入する。application startupからは呼び出さず、staging/productionで既知資格情報を生成しない。
- **`config.py`**(2026-07-13追加、2026-07-14拡張): `APP_ENV`、DB、JWT、CORS、Stripe、SMTP、ログ設定を環境変数から読み取り、型変換と環境別の起動時検査を行う。productionでは既定SECRET_KEY、暗号化されないfrontend/CORS URL、暗黙SQLite、consoleメールを拒否し、有効化した外部機能の資格情報不足も拒否する。他の自作モジュールに依存しない。
...
- **`services/checkout.py`**: 通常注文とStripe決済に共通するカート・在庫・クーポン検証、金額見積もり、注文確定トランザクションを提供する。業務エラーはFastAPIに依存しない例外で表し、ルーターがHTTPエラーへ変換する。
- **`services/order_status.py`**: 注文ステータスの許可値と管理者操作で許可する遷移を管理する。
- **`services/order_actions.py`**: `reverse_order`により、注文取消/返品承認時のStripe返金・在庫復元・クーポン使用数減算を行う。返金失敗はHTTP非依存の`RefundError`で呼び出し元へ通知する。
...
- **`email_utils.py`**: `config.EMAIL_DELIVERY`に従い、SMTP送信、local/test向けconsole出力、または無効化を選択する。productionのconsole選択は`config`が起動時に拒否する。`config`・`logging_config`に依存する。
- **`database.py`**: `config.DATABASE_URL`からSQLAlchemyエンジン・セッション(`get_db`)・`Base`を初期化する。SQLite専用`connect_args`を他dialectへ渡さない。
...
- **`logging_config.py`**: `config.LOG_LEVEL`と共通formatをPython標準loggingへ設定し、named loggerを返す。構造化ログ、相関ID、機密マスキングは未実装。
...
| `POST /orders`, `GET /orders`, `GET /orders/{id}` | `routers/orders.py` | `models`, `schemas`, `auth`, `services/checkout`, `email_utils` |
...
| `GET /config`, `POST /payment/checkout`, `POST /payment/complete` | `routers/payment.py` | `models`, `schemas`, `auth`, `config`, `stripe_client`, `services/checkout`, `email_utils` |
