...
対象: `backend/app/` 配下の全Pythonモジュール(`main.py`, `config.py`, `stripe_client.py`, `routers/*.py`, `services/order_calc.py`, `services/order_actions.py`, `models.py`, `schemas.py`, `auth.py`, `email_utils.py`, `database.py`)。実際の`import`文(2026-07-13時点のソース)に基づいて作成する。
...
- `schemas`・`email_utils`・`config`・`services/order_calc.py` は他の自作モジュールに依存していない(外部ライブラリ・標準ライブラリのみに依存)。
...
- **`main.py`**: FastAPIアプリのコンポジションルート。`app = FastAPI()`の生成、CORSミドルウェア設定、起動時シード処理(`seed_products`/`seed_coupons`/`seed_admin`)、`routers/`配下の各ルーターの`include_router`登録のみを行う。`stripe_client`をimportすることでStripe APIキーの初期化を発火させる以外、状態やロジックを持たない薄いモジュール。
