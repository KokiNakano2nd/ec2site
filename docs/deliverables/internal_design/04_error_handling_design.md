...
- 現時点でログ出力を実装しているのは、外部サービス(Stripe, SMTP)呼び出しの失敗と、決済完了時の不正アクセス試行(`user_id`不一致)の3箇所のみ(NFR-005「決済関連のログ出力」に対応)。それ以外のバリデーションエラー(404/400等)は、クライアントへのレスポンスで原因が明確なため、現状ログ出力の対象外としている。範囲を広げる場合は本ドキュメントを更新すること。
