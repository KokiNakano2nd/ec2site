...
| ERR-PAYMENT-CREATE | `routers.payment.create_checkout_session` | 502 | Checkout Session作成失敗 | `決済サービスとの通信に失敗しました` | ERROR |
| ERR-PAYMENT-RETRIEVE | `routers.payment.complete_payment` | 400 | Session取得失敗 | `決済セッションを確認できませんでした` | ERROR |
...
| 注文確定 | 注文、明細、在庫減算、クーポン回数加算、カート削除 | `services.checkout.place_order`で一括commit/rollback | DB内は同一トランザクション | 注文確認メール失敗はDBを戻さない |
...
トランザクション境界はユースケースを調整するサービスまたはルーターが所有する。注文確定は`place_order`がDB内の全更新を一つのユースケースとしてcommit/rollbackし、下位の計算関数はトランザクションを操作しない。

## 5. 外部副作用・冪等性・競合

| リスクID | 現状 | 影響 | 必要な対策 |
|---|---|---|---|
| RISK-TX-001 | ~~決済確定をブラウザ依存~~ → `POST /payment/webhook`を正経路として実装(2026-07-16)。署名検証・event台帳・5xx時のStripe再試行あり | 実Stripeからの受信はstaging未検証 | staging結合テストで実イベント受信を検証する |
| RISK-TX-002 | `payment/complete`・Webhookの同時/再送 | 重複注文の可能性 | PaymentIntent IDのUNIQUE制約(Alembicで全DBへ反映済み)と既存注文応答、event IDの台帳で対策済み |
| RISK-TX-009 | checkout後にカート・価格・クーポンが変更される | Stripe請求額と注文額・明細の不一致 | 商品ID・数量・単価・coupon・totalの正規化fingerprintをStripe Session metadataへ保存し、`amount_total`とともに確定直前の再計算結果へ照合。不一致は409で注文未作成。支払済み未確定の要照合workflowは未実装 |
...
| RISK-TX-007 | 無効クーポンの決済経路間差異 | 金額・利用者認識の不一致 | 共通checkout serviceへ集約し、通常注文とStripe経路の両方を400へ統一済み |
