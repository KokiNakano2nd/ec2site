...
|---|---|---|
| CUSTOMER | 商品を購入する顧客(ログインユーザー)。管理者もこのエンティティの一種で、`is_admin`フラグにより区別される(`01_table_definition.md`参照)。パスワードリセット用のトークン・有効期限(任意、未設定可)も持つ(2026-07-13追加) | US-001, UC-002, US-006, US-007, UC-009 |
