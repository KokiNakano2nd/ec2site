...
| 画面ID | 画面名 | 概要 | 主なアクター | 元になったドキュメント |
|---|---|---|---|---|
| S-007 | プロフィール画面 | アカウント情報の確認、配送先の登録・削除・デフォルト設定、退会を行う。メールアドレスが未確認の場合はバッジと再送ボタンを表示する(2026-07-13追加、US-027)(`view === "profile"`、`ProfileView`) | 顧客 | US-010, US-011, US-012, US-020, UC-005, US-027, UC-010 |
...
| 画面ID | 画面名 | 概要 | 主なアクター | 元になったドキュメント |
|---|---|---|---|---|
| S-102 | クーポン管理画面 | クーポンの発行・有効化・無効化・削除を行う。クーポンごとに残数アラートしきい値を設定でき、残り使用回数がしきい値以下のクーポンはバッジ表示される(2026-07-13追加、US-025)(`view === "admin-coupons"`、`AdminCouponsView`) | 管理者 | US-016, US-017, US-025 |
