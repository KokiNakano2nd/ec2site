...
describe("AdminDashboardView", () => {
  ...
  it("lists low-stock products when there are any", async () => {
    fetchLowStockProductsMock
      .mockReset()
      .mockResolvedValue([{ id: 1, name: "低在庫商品", stock: 2, low_stock_threshold: 5 }]);
    render(<AdminDashboardView />);
  ...
  it("lists low-remaining-uses coupons when there are any", async () => {
    fetchLowRemainingUsesCouponsMock
      .mockReset()
      .mockResolvedValue([{ id: 1, code: "SUMMER10", max_uses: 10, used_count: 8, low_remaining_uses_threshold: 5 }]);
    render(<AdminDashboardView />);
