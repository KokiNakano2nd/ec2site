      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 28 }}>
        <StatCard
          ...
          value={
            <>
              {orders.length}
              <span style={{ fontSize: 14, color: C.muted, fontWeight: 400, marginLeft: 4 }}>件</span>
            </>
          }
        />
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
            ...
            <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: `1px solid ${C.border}` }}>
              {["注文ID", "注文者", "注文日時", "ステータス", "合計", "返品対応"].map((h, i) => (
                <th
                  key={h}
                  style={{
                    textAlign: i === 3 || i === 5 ? "center" : i === 4 ? "right" : "left",
                    padding: "14px 20px",
                    fontSize: 11,
                    color: C.muted,
                    fontWeight: 700,
            ...
            {orders.map((order) => {
              ...
              return (
                <tr key={order.id} className="admin-row" style={{ borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
                  ...
                  <td style={{ padding: "15px 20px", textAlign: "center" }}>
                    ...
                    >
                      {ORDER_STATUSES.map((st) => (
                        <option key={st.value} value={st.value} style={{ background: C.surface, color: C.text }}>
                          {st.label}
                        </option>
                      ))}
                    </select>
                  ...
                  <td
                    style={{ padding: "15px 20px", textAlign: "right", fontSize: 15, fontWeight: 700, color: C.text }}
                  >
