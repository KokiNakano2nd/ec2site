...
export function ProfileView({ showToast, onAccountDeleted }) {
  ...
  async function handleSetDefault(id) {
    try {
      ...
      setAddresses((prev) =>
        prev.map((a) => ({ ...a, is_default: a.id === id })).sort((a, b) => b.is_default - a.is_default),
      );
  ...
  return (
    <div style={{ animation: "fadeUp 0.3s ease", maxWidth: 720 }}>
      ...
      {error && (
        <ErrorBanner size="sm" style={{ marginBottom: 16 }}>
          {error}
        </ErrorBanner>
      )}
      ...
      {showForm && (
        ...
        >
          ...
          <button className="btn-primary" type="submit" style={{ padding: "10px 24px", fontSize: 13, borderRadius: 8 }}>
            保存する
          </button>
        </form>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {addresses.map((addr) => (
            <div
              ...
              style={{
                background: C.surface,
                border: `1px solid ${addr.is_default ? "rgba(91,139,245,0.35)" : C.border}`,
                borderRadius: 14,
                padding: 20,
                display: "flex",
                ...
                {addr.is_default && (
                  <span
                    style={{
                      fontSize: 10,
                      fontWeight: 700,
                      background: "rgba(91,139,245,0.15)",
                      color: C.accent,
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>
              退会するとログインできなくなり、メールアドレス等の個人情報が匿名化されます。注文履歴は業務記録として残ります。
            </p>
          </div>
          <button
            className="btn-danger"
