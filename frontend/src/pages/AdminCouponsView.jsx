  ...
  async function handleCreate(e) {
    ...
    try {
      const created = await createAdminCoupon(token, {
        ...
        low_remaining_uses_threshold: form.low_remaining_uses_threshold
          ? Number(form.low_remaining_uses_threshold)
          : null,
      });
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
            ...
            <div>
              ...
              <input
                ...
                min="0"
                placeholder="未設定"
                value={form.low_remaining_uses_threshold}
                onChange={(e) => setForm({ ...form, low_remaining_uses_threshold: e.target.value })}
              />
            </div>
          </div>
          <button
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: `1px solid ${C.border}` }}>
              {["コード", "割引内容", "使用回数", "残数アラートしきい値", "ステータス", "作成日", "操作"].map(
                (h, i) => (
                  <th
                    key={h}
                    style={{
          ...
          <tbody>
              ...
              <tr key={coupon.id} className="admin-row" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td
                  style={{
                    ...
                    textAlign: "center",
                    fontSize: 13,
                    color: isLowRemainingUses(coupon) ? C.red : C.sec,
                  }}
                >
                  {coupon.used_count}
                ...
                <td style={{ padding: "15px 20px", textAlign: "center", fontSize: 12, color: C.muted }}>
            ...
            {coupons.length === 0 && (
              <tr>
                <td colSpan={7} style={{ textAlign: "center", padding: "40px 20px", color: C.muted, fontSize: 14 }}>
                  クーポンがありません
                </td>
              </tr>
            )}
